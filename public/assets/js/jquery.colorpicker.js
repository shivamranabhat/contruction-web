$(document).ready(function() {
    ...
    function generateHTML() {
      ...
      for (var palette in palettes) {
        html[palette] = '<div class="colorpicker-palette">\
    ...
    function displayPicker(input) {
      ...
      if ($(parent).find('div').filter('#colorPicker').length == 0) {
  
        $(input).css('z-index','100');
  
        $(input).wrap('<div class="colorPicker-wrapper" style="z-index: 10;" />');
  
        $('.colorPicker-wrapper').append('<div id="colorPicker"></div>');
  
        $('#colorPicker').html(paletteHTML[selection]);
  
        $('li#' + selection).addClass('colorpicker-palette-select');
  
        //reset the default auto value that IE sets (0) to 10 so that the picker works in IE
        $(parent).css('z-index','10');
  
        $('.colorPicker-wrapper .colorPicker').focus();
  
        $('<div/>').attr('class','colorPickerUI').appendTo('body');
      }
    }
    ...
    function cleanPicker() {
      ...
      $('.colorPicker-wrapper').remove();
      $('#colorPicker').remove();
      $('.colorPickerUI').remove();
  
      $('.colorPicker').parent().removeAttr('style');
    ...
    $(document).on("click", '.colorPicker', function () { 
      $('body').bind('click.kp', function (ev) {
        if (!($(ev.target).parents().is(".colorPicker-wrapper") || $(ev.target).is(".colorPicker-wrapper"))) {
          cleanPicker();
        }
      });
    ...
    $(document).on("keypress", '.colorPicker', function (e) {
    ...
    $(document).on("click", '.tile', function(){	
      ...
      var input = $(wrap_div).find('input').filter('.colorPicker');
